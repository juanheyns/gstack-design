...
async function runSetup(): Promise<void> {
  ...
  try {
    await generate({
      ...
      output: resolveOutput(undefined, "smoke-test.png"),
      size: "1024x1024",
      quality: "low",
    });
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "compare": {
      ...
      const outputPath = resolveOutput(flags.output as string, "board.html");
    ...
    case "variants":
      await variants({
        ...
        outputDir: resolveOutputDir(flags["output-dir"] as string, "variants"),
        size: flags.size as string,
        quality: flags.quality as string,
    ...
    case "iterate":
      await iterate({
        ...
        output: resolveOutput(flags.output as string, "iterate.png"),
      });
      break;
    ...
    case "evolve":
      await evolve({
        ...
        brief: flags.brief as string,
        output: resolveOutput(flags.output as string, "evolved.png"),
      });
      break;
    ...
    case "gallery": {
      ...
      gallery({
        designsDir,
        output: resolveOutput(flags.output as string, "gallery.html"),
      });
      break;
    }
